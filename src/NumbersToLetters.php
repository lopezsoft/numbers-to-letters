...
class NumbersToLetters
{
    ...
    public function getNumbersToLetters(float $value = 0, $money = 'PESOS', $money2 = 'CENTAVOS'): string
    {
        $B0         = '';
        $B1         = '';
        $B2         = '';
    ...
    private function readValues(int $number): string
    {
        return match (strlen($number)) {
            1 => $this->getUnits($number),
            2 => $this->getTens($number),
            3 => $this->getHundreds($number),
            default => '',
        };
    }
     ...
     */
    private function getUnits(int $NValue = 0): string
    {
